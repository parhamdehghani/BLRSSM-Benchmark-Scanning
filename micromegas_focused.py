...
while fileno < MaxNumberOfSolution:
    ...
    if LHA.CheckLHAexist(LHA.LHAFileName):
        with open(LHA.LHAFileName, 'r') as file:
            ...
            A0 = SPheno_input.blocks['MINPAR']['5'] 
            SignumMuR = SPheno_input.blocks['MINPAR']['6'] 
            TanBeta = SPheno_input.blocks['MINPAR']['7'] 
            TanBetaR = SPheno_input.blocks['MINPAR']['9']
            VR = SPheno_input.blocks['MINPAR']['11']
            ...
            SPheno_input.blocks['MINPAR']['5'] = A0
            SPheno_input.blocks['MINPAR']['6'] = SignumMuR
            SPheno_input.blocks['MINPAR']['7'] = TanBeta
            SPheno_input.blocks['MINPAR']['9'] = TanBetaR
            SPheno_input.blocks['MINPAR']['11'] = VR
...
############# Check Constraints  ################################################################
            if  (LHA.MassBounds() and LHA.CheckBPhysics() and LHA.CheckWhoIsLSP()):
...
################################################################                      
                    if (good_relic):
                        if (new_relic_density > 0.14 and new_relic_density < 0.16):
                            relic_density = new_relic_density
                            p = subprocess.call("cp LesHouches.in.BLRinvSeesaw_new LesHouches.in.BLRinvSeesaw",shell=True)
                            p = subprocess.call("rm -f LesHouches.in.BLRinvSeesaw_new",shell=True)
                            LHA.Erase(LHA.SLHAFileName)
